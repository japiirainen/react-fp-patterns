...
export const setItemWithEncode = <A>(
   ...
   pipe(item, encode, JSON.stringify, val => setItem(key, val));
   ...
   pipe(
      ...
      RTE.chainW(dataOpt =>
         pipe(
            dataOpt,
            O.fold(
               // Cache miss - do the API call, and store the result in the cache
               () =>
                  pipe(
                     // Do get call
                     get,
                     RTE.chainW(data =>
                        pipe(
                           // store the result as a side-effect
