import { pipe, A, R, Rec, RTE, TE } from '../../util/fpts';
...
export interface BreedImageService<E> {
   breedImageService: (breed: Breed) => TE.TaskEither<E, Array<Image>>;
}
...
/**
 * Reader that takes the required env and produces an implementation of the GetBreedImagesResponse
 */
...
> = env => ({
   breedImageService: breed => getBreedImages(breed)(env),
});
