...
/**
 * Runs an RTE<AppEnv, E, A> and handles before, success, and failure using the given callbacks.
 * The callbacks cannot fail, and are run as IO<void> operations in the RTE chain.
 */
...
export const useAppEnvRTE = <E, A, Deps extends Array<unknown>>({
   ...
   // These should be made more effectful (IO, RT etc...)
   onBefore: () => void;
   onError: (e: E) => void;
   onSuccess: (a: A) => void;
   deps: Deps;
...
/**
 * A hook to handle as RTE effect as RemoteData state
 */
export const useAppEnvRemoteDate = <E, A, Deps extends Array<unknown>>({
   rte,
   deps,
   eqDeps,
}: {
